...
def fitting_new_points_1D(n_units, num_epochs, seed, T, length_scale, n_init, mc_tau, regul, warm_start):
    ...
    x     = np.copy(x_old)
    y     = np.copy(y_old)
    x_aug = np.hstack((x, 0 * np.ones_like(x)))
    x_grid_aug = np.hstack((x_grid, 0 * np.ones_like(x_grid)))
    ...
    for k in range(total_itr):
        ...
        for u in util_set:
            ...
            model_lcbnn_u = LCBNN(num_epochs=num_epochs,n_units=n_units_list,
                                  ...
                                  actv=act_func, normalize_input=False, normalize_output=normalize_input_and_output)
            # Train
            # augment input with k
            lcbnn_train_mse_loss, lcbnn_train_logutil = \
                model_lcbnn_u.train(x_aug, y.flatten(), itr=itr_k, n_per_itr=n_per_update, saving_path=saving_model_path)

            train_time_lcbnn = time.time() - start_train_time_lcbnn

            start_predict_time_lcbnn= time.time()
            # Predict
            m_lcbnn_u, v_lcbnn_u = model_lcbnn_u.predict(x_grid_aug)
        ...
        x_new_aug = np.hstack((x_new, (k+1) * np.ones_like(x_new)))
        x_aug = np.vstack((x_aug,x_new_aug))
