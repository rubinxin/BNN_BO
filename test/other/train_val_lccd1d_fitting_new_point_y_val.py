...
def fitting_new_points_1D(n_units, num_epochs, seed, T, length_scale, n_init, mc_tau, regul, warm_start, util_str):
    ...
    for k in range(total_itr):
        ...
        for epochs in range(10, num_epochs, epoch_interval):
            ...
            # Predict
            m_concdrop, v_concdrop,  = model_concdrop.validate(x_grid, fvals)
            predict_time_mc = time.time() - start_predict_time_mc

            # Validation loss and loglikelihood
            concdrop_val_loss =
            concdrop_val_loglike =

            concdrop_val_loss_allepoch.append(concdrop_val_loss)
            concdrop_val_loglike_allepoch.append(concdrop_val_loglike)
            ...
            for u in util_set:
                ...
                lccd_val_loglike_allepoch.append(lccd_val_loglike)

        # Store the train and prediction loss/log likelihood of the max epoch
        if save_loss:
