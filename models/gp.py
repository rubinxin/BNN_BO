...
# ------------ GP with Likelihood Func Defined Explicitly by ourselves -------- #
class GPModelML(BaseModel):
    ...
    def _update_model(self, X_all, Y_all):
        ...
        # log_opt_params, max_ll = self.MLE(n_start=self.optimize_restarts)
        # self.model.optimizer_array = log_opt_params
    ...
    def _log_likelihood(self, logparams):
        params = np.exp(logparams)

        if self.ARD == True:
            l_scales = params[0:self.input_dim]
            output_var = params[self.input_dim]
            # noise_var = params[self.input_dim + 1]
        else:
            l_scales = params[0]
            output_var = params[1]
            # noise_var = params[2]

        kernel = GPy.kern.Matern52(input_dim=self.input_dim, ARD=self.ARD, variance=output_var, lengthscale=l_scales)
        Kny = kernel.K(self.model.X) + (1e-8) * np.eye(self.model.X.shape[0])

        # compute likelihood terms
        Wi, LW, LWi, W_logdet = pdinv(Kny)
        alpha, _ = dpotrs(LW, self.model.Y, lower=1)
        loglikelihood = 0.5 * (-self.model.Y.size * np.log(2 * np.pi) - self.model.Y.shape[1] * W_logdet
                               - np.sum(alpha * self.model.Y))
        return loglikelihood

    def MLE(self, func_gradient=None, n_start=3):
        hb = np.array([6,6,6])
        lb = np.array([-6,-6,-6])
        bounds = list((li, ui) for li, ui in zip(lb, hb))
        d = len(bounds)
        # get lb and hb ,each are dx1
        log_param_grid = np.tile(lb, (n_start, 1)) + np.tile((hb - lb), (n_start, 1)) * np.random.rand(n_start, d)

        # minimise negative log likelihood
        target_func = lambda x: - self._log_likelihood(x)
        # target_func_gradient = lambda x: func_gradient(x)

        min_nll = 1e6
        log_opt_param = self.model.optimizer_array

        for random_start in log_param_grid:
            if func_gradient:
                x, f_at_x, info = fmin_l_bfgs_b(target_func, random_start, bounds=bounds, approx_grad=False)
        ...
        return np.array([log_opt_param]), maxll

    # def _log_posterior(self, log_params):
    #
    ...
    #     det_l_scales = np.product(params[0:self.input_dim])
    #     logl_priormu = np.log(0.3 * np.ones(self.input_dim))
    #     logl_priorvar = np.diag(3.0 * np.ones(self.input_dim))
    #     prior_l_scales = Gaussianprior(log_params[0: self.input_dim], logl_priormu, logl_priorvar) / det_l_scales
    #     prior_output_var = Gaussianprior(log_params[self.input_dim], np.log(1.0), 3.0) / params[self.input_dim]
    #     piro_noise_var = Gaussianprior(log_params[self.input_dim + 1], np.log(1e-3), 0.1) / params[self.input_dim + 1]
    #
    #     log_posterior = log_likelihood + np.log(prior_l_scales * prior_output_var * piro_noise_var)
