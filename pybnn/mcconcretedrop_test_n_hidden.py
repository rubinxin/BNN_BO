...
class Net(nn.Module):
    def __init__(self, n_inputs, n_units=[50],
        ...
        self.out_mu = nn.Linear(n_units[-1], 1)
    ...
    def forward(self, x):
        ...
        regularization = torch.empty(1, device=x.device)
        ...
        mean, regularization[0] = self.conc_drop_mu(x1, self.out_mu)
...
class MCCONCRETEDROP(BaseModel):
    ...
    def train(self, X, y, itr=0, saving_path = None):
        ...
        dr = 2. / N
        network = Net(n_inputs=features, n_units=self.n_units_list,
        ...
        if itr > 0:
            model_loading_path = os.path.join(saving_path,
                                              f'concdrop_k={itr-1}_{self.actv}_n{self.n_units_list[0]}_e{self.num_epochs}.pt')
        ...
        model_saving_path = os.path.join(saving_path,
                                         f'concdrop_k={itr}_{self.actv}_n{self.n_units_list[0]}_e{self.num_epochs}.pt')
