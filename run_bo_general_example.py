...
for j in range(seed_size):
    ...
    bayes_opt.initialise(X_init=x_init, Y_init=y_init, model_type='GP', batch_size=1, bo_method='EI')

    # output of Bayesian optimisation:
    X_query,Y_query,X_opt,Y_opt = bayes_opt.iteration_step(iterations=10, seed=seed)
    ...
    with open(X_opt_file_name, 'wb') as f:
        pickle.dump(X_opt_all_seeds, f)
    with open(Y_opt_file_name, 'wb') as f:
        pickle.dump(Y_opt_all_seeds, f)
